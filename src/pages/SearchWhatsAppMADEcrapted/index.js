...
const SearchWhatsApp = () => {
    ...
    useEffect(() => {
        setCarregando(true)
        axios.get(`${process.env.REACT_APP_HOST_LOCAL_M}/get-pesquisa`).then((res) => {
    ...
    const Salvar = async () => {
        ...
        setCarregando(true)
        await axios
            .post(`${process.env.REACT_APP_HOST_LOCAL_M}/set-pesquisa`, stateInicial).then((res) => {
                setCarregando(false)
                setAviso({
                    type: 'success',
                    msg: 'Salvo com sucesso!'
    ...
    const Disparar = async () => {
        ...
        await Salvar()
        setCarregando(true)
        axios.get(`${process.env.REACT_APP_HOST_LOCAL_M}/whatsapp-disparar`).then((res) => {
            console.log(res.data)
            setAviso({
                type: 'success',
            ...
            <AppBar position="fixed" sx={{ height: 65 }} >
                <Toolbar>
                    ...
                    <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
                        Pesquisa por WhatsApp - MA
                    </Typography>
                    <Button target="_blank" onClick={() =>  window.open('https://docs.google.com/spreadsheets/d/152BjtpGGy7jgwCSm1S1tPUrc9n2WFxRWWjylAYNPlxQ/edit?usp=sharing',  "_blank")} color="inherit">Planilha</Button>
